...
import {getWhitelistedTokens, getTokenBalances, getUserRegisteredTokens} from "~domain/ref/methods";
import {ftGetBalance, ftGetTokenMetadata, getDepositableBalance} from "~domain/near/ft/methods";
...
export const useFtAssets = () =>{
    ...
    useEffect(()=> {
        view_account_balance(wallet.getAccountId())
            .then(setBalances)
            .catch(e=>{
                console.log("useFtAssets error:", e);
                setBalances({});})
    })
    return balances;
}
...
export const useWhitelistTokens = (extraTokenIds: string[] = []): TokenMetadata[] => {
    const [tokens, setTokens] = useState<TokenMetadata[]>([]);

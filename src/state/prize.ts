...
import {view_prize_pool, view_prize_pool_list} from "~domain/superise/methods";
...
export const usePrizePool= (pool_id:number): PrizePool => {
    const [prizePool, setPrizePool] = useState<PrizePool>(new PrizePool());

    useEffect(() => {
        view_prize_pool(pool_id)
            .then(setPrizePool);
    }, []);
